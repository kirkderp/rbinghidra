...
const OUTPUT_QUERY_VISIBLE_CHARS: usize = 32;
const OUTPUT_QUERY_DIGEST_HEX_CHARS: usize = 16;

#[must_use]
pub fn per_call_output_path(project_dir: &Path, prefix: &str, query: &str) -> PathBuf {
    let sanitized = sanitize_query_for_filename(query);
    let visible_query: String = sanitized.chars().take(OUTPUT_QUERY_VISIBLE_CHARS).collect();
    let digest = query_digest_prefix(query);
    let stamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_or(0, |d| d.as_nanos());
    project_dir.join(format!("{prefix}_{visible_query}_{digest}_{stamp}.json"))
}
...
fn query_digest_prefix(query: &str) -> String {
    ...
    let mut out = String::with_capacity(OUTPUT_QUERY_DIGEST_HEX_CHARS);
    for byte in digest {
        if out.len() >= OUTPUT_QUERY_DIGEST_HEX_CHARS {
            break;
        }
