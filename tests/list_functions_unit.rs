...
use rbinghidra::project::FUNCTIONS_OUTPUT_FILE;
...
fn write_functions_json(
    manager: &rbinghidra::project::ProjectManager,
    sha256: &str,
    program_name: &str,
...
fn returns_not_found_for_unknown_binary_query() {
    ...
    rt.block_on(async {
        ...
        assert!(
            matches!(
                err,
                rbinghidra::list_functions::ListFunctionsError::Inspect(InspectError::NotFound(_))
            ),
...
fn resolve_limit_clamps_to_max() {
    assert_eq!(
        ...
        rbinghidra::list_functions::MAX_LIMIT
    );
}
