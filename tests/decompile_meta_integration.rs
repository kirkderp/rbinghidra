...
use rbinghidra::decompile_meta::{DECOMPILE_META_SCHEMA, DecompileMetaContext, get_decompile_meta};

mod common;

#[test]
#[ignore = "requires real Ghidra/analyzeHeadless; run explicitly with --ignored"]
fn ghidra_decompile_meta_runs_against_real_binary() {
    common::make_runtime().block_on(ghidra_decompile_meta_runs_against_real_binary_impl());
}

async fn ghidra_decompile_meta_runs_against_real_binary_impl() {
