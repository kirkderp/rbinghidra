...
fn discover_install_dir_respects_env_override() {
    ...
    let _install_dir = EnvGuard::set("GHIDRA_INSTALL_DIR", root.as_os_str());
...
fn discover_install_dir_env_override_ignores_invalid() {
    ...
    let tmp = TempDir::new().unwrap();
    let _install_dir = EnvGuard::set("GHIDRA_INSTALL_DIR", tmp.path().as_os_str());
