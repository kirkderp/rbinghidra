...
use rbinghidra::{CachePaths, ServerConfig};
...
fn server_config_uses_defaults_when_env_is_missing_or_invalid() {
    ...
    let _scripts = EnvGuard::remove("RBM_GHIDRA_SCRIPTS_DIR");
...
fn server_config_respects_explicit_env_overrides() {
    ...
    let _scripts = EnvGuard::set("RBM_GHIDRA_SCRIPTS_DIR", "/opt/rbinghidra/scripts");
    ...
    assert_eq!(
        cfg.ghidra_scripts_dir,
        PathBuf::from("/opt/rbinghidra/scripts")
    );
