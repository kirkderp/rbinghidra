...
fn ghidra_callgraph_round_trip_against_real_binary() {
    make_runtime().block_on(ghidra_callgraph_round_trip_against_real_binary_impl());
}

async fn ghidra_callgraph_round_trip_against_real_binary_impl() {
    let health = probe();
    assert!(
        health.available,
