...
use rbinghidra::cfg::{CfgContext, CfgError, gen_cfg};
...
fn ghidra_cfg_round_trip_against_real_binary() {
    make_runtime().block_on(ghidra_cfg_round_trip_against_real_binary_impl());
}

async fn ghidra_cfg_round_trip_against_real_binary_impl() {
    let health = probe();
    assert!(
        health.available,
