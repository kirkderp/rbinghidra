...
use rbinghidra::imports_exports::{ImportsExportsContext, list_exports, list_imports};
...
fn ghidra_imports_exports_round_trip_against_real_binary() {
    make_runtime().block_on(ghidra_imports_exports_round_trip_against_real_binary_impl());
}

async fn ghidra_imports_exports_round_trip_against_real_binary_impl() {
    let health = probe();
    assert!(
        health.available,
