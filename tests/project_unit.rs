...
use rbinghidra::import::{ImportContext, ImportError, ImportReport, import_binary};
...
mod common;
use common::{make_manager, make_runtime, write_executable};
...
#[test]
fn headless_runner_bounds_child_output() {
    let rt = make_runtime();
    rt.block_on(async {
        let tmp = TempDir::new().unwrap();
        let analyze = tmp.path().join("noisy_analyze_headless");
        write_executable(
            &analyze,
        ...
        let outcome = runner.run_import(&make_spec()).await.unwrap();

        assert!(outcome.success);
        assert!(outcome.stdout.contains("earlier output truncated"));
        assert!(outcome.stdout.len() < 1_100_000);
    });
}
...
#[test]
#[cfg(unix)]
fn stage_script_for_headless_skips_current_runtime_copy() {
    ...
    rt.block_on(async {
        ...
        .unwrap();
        let staged_path = runtime_scripts.join(EXTRACT_FUNCTIONS_SCRIPT);
        std::fs::write(&staged_path, b"same payload").unwrap();

        let mut perms = std::fs::metadata(&runtime_scripts).unwrap().permissions();
        perms.set_mode(0o555);
        std::fs::set_permissions(&runtime_scripts, perms).unwrap();
        ...
        assert_eq!(result.unwrap(), staged_path);
    });
}

#[test]
fn stage_script_for_headless_refreshes_stale_runtime_copy() {
    let rt = make_runtime();
    rt.block_on(async {
        let tmp = TempDir::new().unwrap();
        let source_scripts = tmp.path().join("source_scripts");
...
fn hash_file_rejects_directory_paths() {
    ...
    rt.block_on(async {
        ...
        assert!(matches!(
            result,
            Err(rbinghidra::project::ProjectError::NotAFile(_))
        ));
    });
}
