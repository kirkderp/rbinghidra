...
#[test]
fn per_call_output_path_caps_query_segment_and_adds_digest() {
    let project_dir = PathBuf::from("/tmp/proj");
    let query = "a".repeat(300);
    let path = per_call_output_path(&project_dir, "decompile", &query);
    let name = path.file_name().unwrap().to_string_lossy().into_owned();

    assert!(name.starts_with("decompile_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa_"));
    assert!(name.len() < 100, "{name}");
    assert!(!name.contains(&"a".repeat(64)), "{name}");
    assert!(common::has_json_extension(&name), "{name}");
}
