...
pub fn sanitize_project_name(input: &str) -> Cow<'_, str> {
    ...
    // Safe because we only push valid ASCII alphanumeric characters, '_', '-', or '.'
    Cow::Owned(String::from_utf8(cleaned).expect("Cleaned project name should be valid UTF-8"))
}
