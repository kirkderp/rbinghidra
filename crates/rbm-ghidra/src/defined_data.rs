...
use crate::project::{
    DEFINED_DATA_SCRIPT, HeadlessError, PathValidationError, ProjectManager, cache_key,
};
use crate::warm_path::{WarmPathProduct, WarmPathRequest, execute_warm_path};

pub const DEFINED_DATA_SCHEMA: &str = "rbm.ghidra.defined_data.v0";
pub const DEFAULT_QUERY: &str = ".*";
pub const DEFAULT_OFFSET: u64 = 0;
pub const DEFAULT_LIMIT: u64 = 25;
pub const MAX_LIMIT: u64 = 1000;
const OUTPUT_PREFIX: &str = "defined_data";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DefinedDataEntry {
    pub address: String,
    pub label: String,
...
pub enum DefinedDataError {
    ...
    #[error(
        "analyzeHeadless exited successfully but the defined_data postScript produced no output file; stdout: {stdout}; stderr: {stderr}"
    )]
    ...
    #[error("defined_data output at {path} is not valid JSON: {source}")]
    Parse {
        path: PathBuf,
        #[source]
...
/// List defined data entries from a cached Ghidra project.
///
/// # Errors
///
/// Returns an error if the binary cannot be resolved, the Ghidra script cannot
/// run, or the defined-data report cannot be read or decoded.
pub async fn list_defined_data(
    ctx: &DefinedDataContext,
    binary_query: &str,
    query: Option<&str>,
    offset: Option<u64>,
    limit: Option<u64>,
) -> Result<DefinedDataResult, DefinedDataError> {
    ...
    let WarmPathProduct {
        ...
        timeout: ctx.timeout,
        binary_query,
        script_name: DEFINED_DATA_SCRIPT,
    ...
    let envelope: DefinedDataEnvelope =
        serde_json::from_slice(&bytes).map_err(|err| DefinedDataError::Parse {
            path: output_path,
            source: err,
        })?;

    Ok(DefinedDataResult {
